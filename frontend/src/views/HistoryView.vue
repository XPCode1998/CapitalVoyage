      ...
      <section class="logbook-panel">
        ...
        <div v-else class="logbook-list">
          ...
          <article v-for="record in records" :key="record.id" :class="['logbook-ticket',{open:expanded===record.id}]">
            <button class="logbook-main" @click="expanded=expanded===record.id?null:record.id">
            ...
            <Transition name="detail-expand"><dl v-if="expanded===record.id" class="logbook-detail">
...
@media(max-width:1100px){.arrival-board-head{display:none}}@media(max-width:760px){.terminal-code{width:44px;height:44px;font-size:14px}.arrival-heading{align-items:flex-start}.logbook-list{padding:10px}.logbook-ticket::before,.logbook-ticket::after{display:none}}
</style>
